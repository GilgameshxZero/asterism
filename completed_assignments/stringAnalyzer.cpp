...
bool isConsonant(char c) {
	...
	if (
		isTerminator(c) != true && isPunctuation(c) != true && isVowel(c) != true &&
		c <= 'z' && c >= 'a') {
		return true;
	} else {
		return false;
...
int readingLevel(string Total) {
	int userreadingLevel =
		(numVowels(Total) + numConsonants(Total)) / numWords(Total);
	return userreadingLevel;
}
double wofcostMetric(string Total2) {
	double userwofcostMetric =
		(numVowels(Total2) * 1.0) / (numWords(Total2) * 1.0);
...
int main(int argc, const char *argv[]) {
	...
	while (userInput != "done") {
		...
		cout << "The total number of sentences is: " << numSentences(userInput)
				 << endl;
		cout << "The total number of vowels is: " << numVowels(userInput) << endl;
		cout << "The total number of consonants is " << numConsonants(userInput)
				 << endl;
		cout << "The total reading level is " << readingLevel(userInput) << endl;
		cout << "The wheel-of-fortune cost metric is " << wofcostMetric(userInput)
				 << endl;
		cout
			<< "Please enter another sentence/or set of sentences for evaluation.\n";
	}
	return 0;
} //staff solution is about 80 lines of code. How can I make this more efficient?