...
//NOT using namespace std:: so make sure it's added to all the strings, vectors, etc.

//we ONLY want to make a deck of cards. only one function needed, and that is to assemble the deck
...
std::vector<Card> makeDeck() {
    ...
    while ( count <= 52) {
        ...
        for (int a = 13; a < 26 ; a++ ){ //start at index 13, since indices 0-12 are full of allclubs
            Card allspades;
            allspades.rank = a + 1;
            allspades.theSuit.spade = 2;
            finishedDeck.push_back(allspades);
        }
        for (int a = 26; a < 39 ; a++ ){ //start at index 26, 13-25 are now full of allspades
            Card alldiamonds;
            alldiamonds.rank = a + 1;
            alldiamonds.theSuit.diamond = 3;
            finishedDeck.push_back(alldiamonds);
        }
        for (int a = 39; a <= 52 ; a++ ){ //start at index 39, 26-38 are now full of alldiamonds
                Card allhearts;
                allhearts.rank = a + 1;
                allhearts.theSuit.heart = 3;
