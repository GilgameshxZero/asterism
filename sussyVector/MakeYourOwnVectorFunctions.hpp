//
//  MakeYourOwnVectorFunctions.hpp
//  MakeYourOwnVector
//
...
class vectorStruct {
	private:
	T *addressArray;	//= new int[]; //a pointer of type T to the address of the
										// array //if I dereferenced it, it would just return the
										// first element in the array. Don't dereference the pointer
										// if you want the address lol.
	...
	/*
	 ...
	 >). These should compare vectors lexicographically (which means in dictionary
	 order). Remember, only 2 of the comparison operators need "real code" in them
	 - the others can use just those two.
	...
	// don't need vectorStruct:: in front because these declarations are within
	// the class vectorStruct:: could write all the functions here, within the
	// class, but they are in a .cpp file because prof. Dave wanted us to learn
	// them that way first

	vectorStruct<T> &operator=(vectorStruct<T> const &otherVector);
	// vectorStruct<T> &vectorStruct<T>::operator=(vectorStruct &otherVector);
	//
	// int &x, &y;
	// T& vectorStruct<T>::operator= (vectorStruct& otherVector);
	T const &operator[](
		int index) const;	 // const overload means this function does not change any
	...
	// vectorStruct<int> const x;
	// x[3] = 1; // not possible because this calls the second overload
	// int y = x[3];

	vectorStruct(int newCapacity);
	// constructor, makes a vector of capacity
	// size  "capacity"
	vectorStruct(const vectorStruct<T> &myVector);	// copy constructor
	~vectorStruct();

	// void growVector(vectorStruct &myVector); //C version
	void growVector();
	// in C, it was necessary to pass a reference to a copy of an
	// object as an argument because classes didn't exist, but with
	// a class in C++, "this" points to itself, so it doesn't need
	// to pass ANY arguments. It points to itself and acts on itself
	void pushBack(T const &newValue);
	void popBack();
	T &get(int index);
	void set(int index, T const &newValue);
...
vectorStruct<T>::vectorStruct(int newCapacity) {
	...
	// allocatable array size_t specifies how big something is, unsigned look it
	// up
...
void vectorStruct<T>::set(int index, T const &newValue) {
	...
	// }
	if (index >= arraySize || index < 0) {
		throw std::runtime_error("Error! Value at index nonexistant. Die now!\n");
	}
